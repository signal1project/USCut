...
| `c425e50` | **Subscription entitlement.** `commont/entitlement.ts`: ed25519-signed compact tokens (`base64url(claims).base64url(sig)`). The app bundles **only the public key** (`electron/main/licensing/entitlement.ts`) and verifies **offline**. `resolveLicenseStatus`: active → grace (14-day offline window past the paid period) → expired → invalid; `isPremiumUnlocked`. Token + last-verified timestamp persisted in `Settings` (signed, not a secret). IPC `license:status` / `license:activate` / `license:deactivate`. Settings page gets a **Subscription card** (`src/views/mas/SubscriptionCard.tsx`). **Feature gating is NOT wired** — status is shown, nothing is locked — pending Dale's tier decisions. `services/licensing/` started as a scaffold here — **superseded by `953e853`**, see the late-update section: it now has real Stripe SDK webhook verification, live subscription-state reconciliation, a persistent idempotent store, and JWKS auth. Still not connected to a live Stripe account or deployed. |
...
| 2 | **Stripe + identity + deploy** | The `services/licensing` service is **built** (SDK webhook verification, live subscription-state reconciliation, persistent idempotent store, JWKS verified-email `/activate`, full HTTP round-trip test — see the late-update section). Dale must supply: a Stripe account + test-mode keys (`STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_IDS`); the price/currency; the **private** signing key at `C:\home\dalebrown138\projects\_secrets\uscut-license.private.pem` (**outside the repo — never commit it**; matching public key in `electron/main/licensing/entitlement.ts`) as `LICENSE_SIGNING_KEY`; a real identity provider (`AUTH_ISSUER` / `AUTH_AUDIENCE` / `AUTH_JWKS_URL`); a host + persistent volume for `LICENSE_STORE_PATH` (the JSON store is single-writer — a clustered deploy needs a real DB behind the same interface); the webhook registration; **the in-app customer checkout + billing-portal UI**; and the **app-side feature-gating decision** (tiers — nothing is locked today). |
...
- **connect the licensing service to real Stripe test mode** once Dale gives you keys — the code seams are done; you'd add `.env`, run it against Stripe's CLI (`stripe listen --forward-to localhost:8791/webhook`), and confirm a real test subscription issues a token the app activates. Then build the **customer checkout page** (Stripe Checkout redirect) and a **billing-portal** link in the app.
- **wire subscription feature-gating** after Dale defines the tiers (`isPremiumUnlocked(status)` already exists in `electron/main/licensing/entitlement.ts` — the seam is `license:status`; gate the premium surfaces in the renderer).
...
- **dependency-security follow-ups** (`docs/SECURITY-AUDIT-2026-09-09.md`): bump `sharp`/`@huggingface/transformers`/`kokoro-js` when the libvips/libheif fix lands; the `electron` 33 → current upgrade is a scoped task (native ABI rebuild for `better-sqlite3` + `onnxruntime-node`, API-change sweep, full re-test + re-smoke); sweep the build-tool findings with per-package compat checks (never `--force`).
- do the one-click-caption durable job if Dale wants it; further trim the installer.

**Every change:** `npx tsc --noEmit` + `npx vitest run` (expect **553**/565) + `npm run lint:eslint` + `npx vite build` + `node scripts/smoke-stabilization.mjs` before claiming done. Licensing-service changes also need `cd services/licensing && npm test` (offline lifecycle + 9 `node --test` cases). For anything touching packaging or native/runtime paths, also `npm run rebuild && npx electron-builder --win nsis --publish never` and `node scripts/smoke-stabilization.mjs "release/0.1.0/win-unpacked/USCut.exe"`. Commit at the end of every clean build (split by feature, honest verification tier in the message); push only on Dale's per-task go-ahead.
