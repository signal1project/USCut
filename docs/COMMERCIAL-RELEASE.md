...
| Full AI workflow | Provider resolver, AI storyboard generation, frame-grounded planning (vision-capable providers), single-scene AI revision, durable/cancellable Auto-Edit and one-click caption jobs (2026-09-16) | Generated footage + usage/spending controls remain |
| Subscription checkout and access | Offline-verifiable ed25519 tokens (app verify + strict claim validation + 14-day grace + Settings activate/deactivate); `services/licensing` **implemented** — Stripe SDK webhook verification, live subscription-state reconciliation (price-restricted, never invents paid time), persistent idempotent store, JWKS verified-email `/activate`; full HTTP round-trip test; app-side **feature gating wired 2026-09-16** (`electron/main/licensing/guard.ts` gates Studio AI, Auto-Edit/Captions/Clip, ElevenLabs narration, AI ad-copy, publish/schedule) | Blocked on Dale: Stripe account + test keys, deployed private signing key, real identity provider (`AUTH_*`), persistent volume, the checkout + billing-portal UI |
